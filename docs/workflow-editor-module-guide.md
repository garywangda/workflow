...
- 配置状态：`Needs setup` 以标题区最右侧的纯文字显示，不使用圆点、胶囊、按钮或独立状态栏；不显示 `Pending`、`Running`、`Completed`、`Failed` 等任何运行状态，也不在节点上展开完整 Inspector 配置。
...
- 仅保留一个不可见的顶部输入锚点，让来自 Condition 的连线准确落在菱形顶点；不显示连接圆点或添加点，也不提供底部输出点。
