...
- 主体字段：按具体节点名称、负责人/Provider、最多两行的简短描述排列。
- 配置状态：`Needs setup` 以标题区最右侧的橙色 `#F73803` 提示文字显示，不使用圆点、胶囊、按钮或独立状态栏；不显示 `Pending`、`Running`、`Completed`、`Failed` 等任何运行状态，也不在节点上展开完整 Inspector 配置。
- 四连接点：上、右、下、左各一个 React Flow Handle；命中区域与视觉圆点由 `workflow-canvas.css` 统一控制，可见圆点外缘与节点边框保留约 5px 净距。
...
`ConditionBranchNode` 用于表达 Condition 的直接路由结果，例如 `Yes` 和 `No`。它不是独立执行节点，也不进入 Node Library 或 Inspector：

- 使用紧凑的黄色菱形，只显示分支标签，不显示名称、描述、负责人、配置状态或运行状态。
- 顶部仅保留不可见的输入锚点，让来自 Condition 的连线准确落在菱形顶点且不显示添加点；左、右、下三个方向提供与普通节点一致的悬浮加号，可继续连接后续节点。
- 子节点本身不可选择，避免被误认为拥有独立配置；移动和删除仍由 Canvas 的统一节点机制管理。
- 当前 Canvas 中的 Condition → Yes / No 仅为静态视觉示例，不包含条件编辑、自动生成分支或分支增删交互。
